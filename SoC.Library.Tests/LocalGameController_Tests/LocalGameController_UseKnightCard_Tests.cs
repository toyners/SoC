...
namespace Jabberwocky.SoC.Library.UnitTests.LocalGameController_Tests
{
    ...
    public class LocalGameController_UseKnightCard_Tests : LocalGameControllerTestBase
    {
        ...
        public void Scenario_LargestArmyEventsRaisedWhenBothPlayerAndOpponentPlayTheMostKnightDevelopmentCards()
        {
            ...
            gameEvents.Count.ShouldBe(36);
            gameEvents[0].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[1].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            ...
            gameEvents[3].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[4].ShouldContainExact(new GameEvent[] { expectedSecondDiceRollEvent });
            gameEvents[5].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[6].ShouldContainExact(new GameEvent[] { expectedThirdDiceRollEvent });
            gameEvents[7].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[8].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[9].ShouldContainExact(new GameEvent[] { expectedFirstDiceRollEvent, expectedPlayKnightCardEvent });
            gameEvents[10].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[11].ShouldContainExact(new GameEvent[] { expectedSecondDiceRollEvent });
            gameEvents[12].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[13].ShouldContainExact(new GameEvent[] { expectedThirdDiceRollEvent });
            gameEvents[14].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[15].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[16].ShouldContainExact(new GameEvent[] { expectedFirstDiceRollEvent, expectedPlayKnightCardEvent });
            gameEvents[17].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[18].ShouldContainExact(new GameEvent[] { expectedSecondDiceRollEvent });
            gameEvents[19].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[20].ShouldContainExact(new GameEvent[] { expectedThirdDiceRollEvent });
            gameEvents[21].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[22].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[23].ShouldContainExact(new GameEvent[] { expectedFirstDiceRollEvent, expectedPlayKnightCardEvent });
            gameEvents[24].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[25].ShouldContainExact(new GameEvent[] { expectedSecondDiceRollEvent });
            gameEvents[26].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[27].ShouldContainExact(new GameEvent[] { expectedThirdDiceRollEvent });
            gameEvents[28].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[29].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[30].ShouldContainExact(new GameEvent[] { expectedFirstDiceRollEvent, expectedPlayKnightCardEvent, expectedDifferentPlayerHasLargestArmyEvent });
            gameEvents[31].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[32].ShouldContainExact(new GameEvent[] { expectedSecondDiceRollEvent });
            gameEvents[33].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[34].ShouldContainExact(new GameEvent[] { expectedThirdDiceRollEvent });
            gameEvents[35].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
        }
        ...
        public void Scenario_LargestArmyEventOnlyReturnedFirstTimeThatOpponentHasMostKnightCardsPlayed()
        {
            ...
            gameEvents[21].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[22].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[23].ShouldContainExact(new GameEvent[] { expectedFirstDiceRollEvent, expectedPlayKnightCardEvent, expectedDifferentPlayerHasLargestArmyEvent });
            gameEvents[24].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[25].ShouldContainExact(new GameEvent[] { expectedSecondDiceRollEvent });
            gameEvents[26].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[27].ShouldContainExact(new GameEvent[] { expectedThirdDiceRollEvent });
            gameEvents[28].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[29].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[30].ShouldContainExact(new GameEvent[] { expectedFirstDiceRollEvent, expectedPlayKnightCardEvent });
            gameEvents[31].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[32].ShouldContainExact(new GameEvent[] { expectedSecondDiceRollEvent });
            gameEvents[33].ShouldContainItems(expectedPlayerCollectedResources, expectedFirstOpponentCollectedResources);
            gameEvents[34].ShouldContainExact(new GameEvent[] { expectedThirdDiceRollEvent });
        ...
        public void Scenario_OpponentHasEightVictoryPointsAndBuildsLargestArmyToWin()
        {
            ...
            // Assert
            var expectedGameWinEvent = new GameWinEvent(firstOpponent.Id, firstOpponent.VictoryPoints);
